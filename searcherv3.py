...
def main():
  ...
  if (choice == '1'):
    listword = open(input(fb+'ListWords : '), 'r').read().replace('\n', ' ').strip()
    lis = listword.split(' ')
    pp = Pool(50)
    pp.map(dorker, lis)
  elif (choice == '2'):
    plugins = input('enter plugins name : ')
    get_plugin(plugins)
  elif (choice == '3'):
    themes = input('enter themes name : ')
    get_theme(themes)
  elif (choice == '4'):
    tld = input('enter Tld. Domain : ')
    zone(tld)
  elif (choice == '5'):
    dates = input(fr+'input date (yyyy-mm-dd): ')
    grab(dates)
  elif (choice == '6'):
    domz = open(input(fb+"Give Me list to get IP's : "),'r').read().replace('http://', '').replace('https://', '').splitlines()
    Thread = input('Thread:~# ')
    th = Pool(int(Thread))
    th.map(domaintoip, domz)
  elif (choice == '7'):
    zizi = input(fg+'List Ips:~#')
    with open(zizi) as xy:
      for ipz in xy:
        ipranger(ipz)
  elif (choice == '8'):
    target = open(input(fw+'List:~# '),'r').read().replace('http://', '').replace('https://', '').splitlines()
    Threadz = input(fw+'Thread:~# ')
    xx = Pool(int(Threadz))
    xx.map(revip, target)
  elif (choice == '9'):
    tdomain = open(input(fw+'Input List:~# '),'r').read().replace('http://', '').replace('https://', '').splitlines()
    Threadz = input(fw+'Thread:~# ')
    xz = Pool(int(Threadz))
